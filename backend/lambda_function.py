...
class main:
  ...
  def __init__(self, gsheet=True, whoop=True):
    self.CRED_PATH = 'backend/creds.ini'
    if whoop: self.whoop_df = self.get_whoop_data()
    if gsheet: self.gsheet_df = self.get_gsheet_data()
  ...
  def gsheet_output(self):
    pd.set_option("display.max_rows", None, "display.max_columns", None)
    print('----------------------')
    print('all types')
    # print(self.gsheet_df)
    # print(self.gsheet_df.corr())
    print(self.gsheet_df.dtypes)
...
main = main()
if whoop and gsheet: main.analyze()
if gsheet and not whoop: main.gsheet_output()